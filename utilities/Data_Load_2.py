...
def sent_eos_label(docu):
    eos_position = [pos for pos, eos in enumerate(docu) if eos=="</s>"]
    #eos_label = [1 if word=="</s>" else 0 for word in docu]
    #eos_position = [pos for pos, eos in enumerate(eos_label) if eos==1]
    #eos_position = [pos-1 for pos, eos in enumerate(eos_label) if eos==1]
    #return eos_label
    return eos_position
...
def Batch_Preprocessing(batch, word2idx, batch_size, batch_first=True, pad_token="<pad>"):
    ...
    #To handle the end case where batch size can be < 10, so padding is done
...
class Document_Batching_Unpaired():
    """
    - get a document and return sentences
    """
    def __init__(self, doc_batch, label, eos, batch_len, pad_idx, args, data_name):
        self.docs = doc_batch
    ...
    def __iter__(self):
        ...
        for s_i in range(max_doc_len):
            try: 
                sent_batch = [docu for docu in self.docs]
                labels = [lab for lab in self.label]
                eos_list = [_eos for _eos in self.eos]
            except:
