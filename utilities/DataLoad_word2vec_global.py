...
def Order_Creator_Global(pos_batch, neg_batch, batch_docs_len, device):
    label = []
    max_len = max(batch_docs_len)
    for pos_doc, neg_doc in zip(pos_batch, neg_batch):
        label_temp = []
...
class Batch_Generator_Insertion():
    ...
    def __iter__(self):
        ...
        insertion_item = [data+".pos.text" for data in insertion_item]

#        for datafile in os.listdir(self.dirname):
#            insertion_item_list = []
#            for data in insertion_item:
#                if fnmatch.fnmatch(datafile, str(data)+'*'):
#                    insertion_item_list.append(datafile)
#                    continue
#        pdb.set_trace()  
#        print(f"insertion_item_list: {insertion_item_list}") 
